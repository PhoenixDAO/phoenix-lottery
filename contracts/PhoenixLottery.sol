...
// PhoenixToken: 0x2df33c334146d3f2d9e09383605af8e3e379e180
// IdentityRegistry: 0xa7ba71305bE9b2DFEad947dc0E5730BA2ABd28EA
// Most recent PhoenixLottery deployed address: 0x001328288dd358644e289f48ec4ef0bc3139a2d2
...
contract PhoenixLottery {
    ...
    struct Lottery {
    	...
    	bytes32 name;
    	string description;
    	uint256 phoenixPrice;
        uint256 phoenixReward;
    ...
    constructor(address _identityRegistryAddress, address _tokenAddress, address _randomizer) public {
        ...
        phoenixToken = PhoenixTokenTestnetInterface(_tokenAddress);
        identityRegistry = IdentityRegistryInterface(_identityRegistryAddress);
        randomizer = RandomizerInterface(_randomizer);
    }

    /// @notice Defines the lottery specification requires a PHOENIX payment that will be used as escrow for this lottery. The escrow is a separate contract to hold people’s PHOENIX funds not ether. Remember to approve() the right amount of PHOENIX for this contract to set the phoenix reward for the lottery.
    /// @param _name The lottery name
    /// @param _description What the lottery is about
    /// @param _phoenixPricePerTicket How much each user has to pay to participate in the lottery, the price per ticket in PHOENIX
    /// @param _phoenixReward The PHOENIX reward set by the owner of the lottery, the one that created it. Those are the tokens that the winner gets
    ...
    function createLottery(bytes32 _name, string memory _description, uint256 _phoenixPricePerTicket, uint256 _phoenixReward, uint256 _beginningTimestamp, uint256 _endTimestamp, uint256 _fee, address payable _feeReceiver) public returns(uint256) {
        ...
        require(phoenixToken.balanceOf(msg.sender) >= _phoenixReward, 'You must have enough token funds for the reward');
        require(_phoenixReward > 0, 'The reward must be larger than zero');
        ...
        // Creating the escrow contract that will hold PHOENIX tokens for this lottery exclusively as a safety feature
        PhoenixEscrow newEscrowContract = new PhoenixEscrow(_endTimestamp, address(phoenixToken), _phoenixReward, _fee, _feeReceiver);
        escrowContracts[address(newEscrowContract)] = newLotteryId;
        escrowContractsArray.push(address(newEscrowContract));

        uint256 allowance = phoenixToken.allowance(msg.sender, address(this));
        // Transfer PHOENIX tokens to the escrow contract from the msg.sender's address with transferFrom() until the lottery is finished
        // Use transferFrom() after the approval has been manually done. Checking the allowance first.
        require(allowance >= _phoenixReward, 'Your allowance is not enough. You must approve() the right amount of PHOENIX tokens for the reward.');
        require(phoenixToken.transferFrom(msg.sender, address(newEscrowContract), _phoenixReward), 'The token transfer must be successful');
        ...
        Lottery memory newLottery = Lottery({
            ...
            phoenixPrice: _phoenixPricePerTicket,
    ...
    function buyTicket(uint256 _lotteryNumber) public returns(uint256) {
        ...
        require(phoenixToken.transferFrom(msg.sender, escrowContract, ticketPrice), 'The ticket purchase for this lottery must be successful when transfering tokens');
