...
import './PhoenixLotteryInterface.sol';
import './usingOraclize.sol';

// Remember to setup the address of the phoenixLottery contract before using it

// Create a contract that inherits oraclize and has the address of the phoenix lottery
// A function that returns the query id and generates a random id which calls the phoenix lottery
contract Randomizer is usingOraclize {
    ...
    event SetPhoenixLotteryAddress(address _phoenixLottery);

    PhoenixLotteryInterface public phoenixLottery;
    ...
    modifier onlyPhoenixLottery {
        require(msg.sender == address(phoenixLottery), 'This function can only be executed by the Phoenix Lottery smart contract');
        _;
    }
    ...
    /// @notice Set the address of the phoenix lottery contract for communicating with it later
    /// @param _phoenixLottery The address of the lottery contract
    function setPhoenixLottery(address _phoenixLottery) public onlyOwner {
        require(_phoenixLottery != address(0), 'The phoenix lottery address can only be set by the owner of this contract');
        phoenixLottery = PhoenixLotteryInterface(_phoenixLottery);
        emit SetPhoenixLotteryAddress(_phoenixLottery);
    }
    ...
    function startGeneratingRandom(uint256 _maxNumber) public payable onlyPhoenixLottery returns(bytes32) {
   ...
   function __callback(
      ...
      phoenixLottery.endLottery(_queryId, parseInt(_result));
   }
}
